Компонент `PeriodPicker` принимает в `value` строку формата `dd.mm.yyyy`.
При вводе строки в поле ввода, возвращает строку без маски.

Имеется статический метод `PeriodPicker.validate`, который проверяет,
что введенная дата корректна

```ts static
PeriodPicker.validate: (value: string, range?: { minDate?: string; maxDate?: string }) => boolean
```
...
const validate = () => {
  const errorNew = !!value && !PeriodPicker.validate(value, { minDate: minDate, maxDate: maxDate });
  setError(errorNew);
  setTooltip(errorNew);
};
...
Очистить значение в `PeriodPicker`'е можно с помощью пустой строки, `null` или `undefined`
...
```jsx harmony
import * as PeriodPickerHelpers from './PeriodPickerHelpers';
...
const createRandomHolidays = () => {
  ...
  for (let index = 0; index < holidays.length; index++) {
    ...
    holidays[index] = PeriodPickerHelpers.formatDate(holiday);
  }
...
<PeriodPicker isHoliday={isHoliday} value={value} onValueChange={setValue} enableTodayLink />;
```
...
<PeriodPicker isHoliday={isHoliday} onValueChange={() => void 0} enableTodayLink />;
```
...
class PeriodPickerFormatting extends React.Component {
  constructor() {
    this.state = {
      order: DateOrder.YMD,
      separator: 'Dot',
  ...
  render() {
    return (
      <Gapped vertical gap={10}>
        ...
        >
          <PeriodPicker onValueChange={value => this.setState({ value })} value={this.state.value} />
        </LocaleContext.Provider>
      </Gapped>
    );
  }
...
<PeriodPicker value={value} onValueChange={setValue} renderDay={renderDay} />;
```

#### Локали по умолчанию

```typescript static
interface PeriodPickerLocale {
  today?: string;
  months?: string[];
  order?: DateOrder;
...
На мобильных устройствах есть несколько вариантов. По умолчанию откроется адаптивная версия в попапе. Но можно открывать нативный календарь, если передать проп `useMobileNativePeriodPicker`.
