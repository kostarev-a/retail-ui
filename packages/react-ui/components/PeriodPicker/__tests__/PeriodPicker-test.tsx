...
import { PeriodPicker, PeriodPickerDataTids } from '../PeriodPicker';
import { PeriodPickerLocaleHelper } from '../locale';
import { LangCodes, LocaleContext } from '../../../lib/locale';
import { componentsLocales as PeriodPickerLocalesRu } from '../locale/locales/ru';
import { componentsLocales as PeriodPickerLocalesEn } from '../locale/locales/en';
...
describe('PeriodPicker', () => {
  describe('validate', () => {
    const { minDate, maxDate } = PeriodPicker.defaultProps;
    it(`should validate by default range ${minDate} - ${maxDate}`, () => {
      expect(PeriodPicker.validate(minDate)).toBe(true);
      expect(PeriodPicker.validate(maxDate)).toBe(true);
      expect(PeriodPicker.validate(new InternalDate({ value: minDate }).shiftYear(-1).toString())).toBe(false);
      expect(PeriodPicker.validate(new InternalDate({ value: maxDate }).shiftYear(1).toString())).toBe(false);
    });
    it('should validate by limits', () => {
      expect(PeriodPicker.validate('00.00.1900', { minDate: '01.01.1800' })).toBe(false);
      expect(PeriodPicker.validate('99.99.2018', { maxDate: '01.01.2019' })).toBe(false);
    });
    it('should validate by number', () => {
      expect(PeriodPicker.validate('01.ff.2019')).toBe(false);
    });
  });

  it('renders', () => {
    render(<PeriodPicker value="02.07.2017" onValueChange={jest.fn()} />);
    expect(screen.getByTestId(PeriodPickerDataTids.label)).toBeInTheDocument();
  });

  it('renders date select when open', () => {
    render(<PeriodPicker value="02.07.2017" onValueChange={jest.fn()} />);
    userEvent.click(screen.getByTestId(PeriodPickerDataTids.input));

    expect(screen.getByTestId(CalendarDataTids.root)).toBeInTheDocument();
  });

  it("doesn't open on focus if disabled", () => {
    render(<PeriodPicker value="02.07.2017" onValueChange={jest.fn()} disabled />);
    userEvent.click(screen.getByTestId(PeriodPickerDataTids.input));
  ...
  it('closes when become disabled', () => {
    const { rerender } = render(<PeriodPicker value="02.07.2017" onValueChange={jest.fn()} />);
    userEvent.click(screen.getByTestId(PeriodPickerDataTids.input));
    expect(screen.getByTestId(CalendarDataTids.root)).toBeInTheDocument();

    rerender(<PeriodPicker value="02.07.2017" onValueChange={jest.fn()} disabled />);
    expect(screen.queryByTestId('Calendar')).not.toBeInTheDocument();
  });

  it('open when autoFocus enabled', () => {
    render(<PeriodPicker value="02.07.2017" onValueChange={jest.fn()} autoFocus />);
    expect(screen.getByTestId(CalendarDataTids.root)).toBeInTheDocument();
  });

  it('blur() methon works', () => {
    const PeriodPickerRef = React.createRef<PeriodPicker>();
    render(<PeriodPicker value="02.07.2017" onValueChange={jest.fn()} ref={PeriodPickerRef} />);
    userEvent.click(screen.getByTestId(PeriodPickerDataTids.input));
    expect(screen.getByTestId(CalendarDataTids.root)).toBeInTheDocument();

    PeriodPickerRef.current?.blur();
  ...
  it('handle onBlur event', () => {
    ...
    render(<PeriodPicker value="02.07.2017" onValueChange={jest.fn()} ref={PeriodPickerRef} onBlur={onBlur} />);
    userEvent.click(screen.getByTestId(PeriodPickerDataTids.input));
    expect(screen.getByTestId(CalendarDataTids.root)).toBeInTheDocument();
  ...
  it('handle onFocus event', () => {
    ...
    render(<PeriodPicker value="02.07.2017" onValueChange={jest.fn()} onFocus={onFocus} />);
    userEvent.click(screen.getByTestId(PeriodPickerDataTids.input));
  ...
  it('renders day cells with renderDay prop', async () => {
    ...
    render(
      <PeriodPicker value="02.07.2017" onValueChange={jest.fn()} renderDay={(props) => <CustomDayItem {...props} />} />,
    );
    await userEvent.click(screen.getByTestId(PeriodPickerDataTids.input));
  ...
  it('onMonthChange returns correct month', async () => {
    ...
    render(<PeriodPicker value={'02.06.2017'} onValueChange={jest.fn()} onMonthChange={onMonthChange} />);

    userEvent.click(screen.getByTestId(PeriodPickerDataTids.input));
    userEvent.click(
      screen.getByRole('button', {
        name: `${DateSelectLocalesRu.selectChosenAriaLabel} ${DateSelectLocalesRu.selectMonthAriaLabel} ${
          PeriodPickerLocaleHelper.get(LangCodes.ru_RU).months?.[5]
        }`,
      }),
    );
    userEvent.click(
      screen.getByRole('button', {
        name: `${DateSelectLocalesRu.selectChooseAriaLabel} ${DateSelectLocalesRu.selectMonthAriaLabel} ${
          PeriodPickerLocaleHelper.get(LangCodes.ru_RU).months?.[6]
        }`,
      }),
    );
  ...
  describe('Locale', () => {
    ...
    it('render without LocaleProvider', () => {
      render(<PeriodPicker value="02.07.2017" onValueChange={jest.fn()} enableTodayLink />);
      const expectedText = PeriodPickerLocaleHelper.get(defaultLangCode).today;
    ...
    it('render default locale', () => {
      render(
        <LocaleContext.Provider value={{ locale: {}, langCode: defaultLangCode }}>
          <PeriodPicker value="02.07.2017" onValueChange={jest.fn()} enableTodayLink />
    ...
    it('render correct locale when set langCode', () => {
      ...
      const expectedText = PeriodPickerLocaleHelper.get(LangCodes.en_GB).today;
    ...
    it('render custom locale', () => {
      render(
        <LocaleContext.Provider
          value={{ locale: { PeriodPicker: { separator: InternalDateSeparator.Dash } }, langCode: LangCodes.en_GB }}
        >
          <PeriodPicker value="02.07.2017" onValueChange={jest.fn()} enableTodayLink />
        </LocaleContext.Provider>,
      );
    ...
    it('should rename months using locale', () => {
      ...
      render(
        <LocaleContext.Provider value={{ locale: { PeriodPicker: { months: renamedMonths } } }}>
          <PeriodPicker value="12.06.2022" onValueChange={jest.fn()} />
        </LocaleContext.Provider>,
      );
    ...
    it.each(['', null, undefined])('should clear the value when %s passed', (testValue) => {
      const Comp = () => {
        ...
        return (
          <>
            <PeriodPicker value={value} onValueChange={setValue} />
    ...
    it('should have disabled input', () => {
      render(<PeriodPicker onValueChange={jest.fn()} disabled />);
  ...
  describe('a11y', () => {
    it('sets value for aria-label attribute (ru)', () => {
      render(<PeriodPicker enableTodayLink onValueChange={jest.fn()} />);

      userEvent.click(screen.getByTestId(PeriodPickerDataTids.input));

      expect(screen.getByTestId(PeriodPickerDataTids.pickerTodayWrapper)).toHaveAttribute(
        'aria-label',
        PeriodPickerLocalesRu.todayAriaLabel,
      );
    });
    ...
    it('sets value for aria-label attribute (en)', () => {
      render(
        <LocaleContext.Provider value={{ langCode: LangCodes.en_GB }}>
          <PeriodPicker enableTodayLink onValueChange={jest.fn()} />
        </LocaleContext.Provider>,
      );
      ...
      expect(screen.getByTestId(PeriodPickerDataTids.pickerTodayWrapper)).toHaveAttribute(
        'aria-label',
        PeriodPickerLocalesEn.todayAriaLabel,
      );
    });
    ...
    it('sets custom value for `todayAriaLabel` locale', () => {
      ...
      render(
        <LocaleContext.Provider value={{ locale: { PeriodPicker: { todayAriaLabel: customAriaLabel } } }}>
          <PeriodPicker enableTodayLink onValueChange={jest.fn()} />
        </LocaleContext.Provider>,
      );

      userEvent.click(screen.getByTestId(PeriodPickerDataTids.input));
      expect(screen.getByTestId(PeriodPickerDataTids.pickerTodayWrapper)).toHaveAttribute('aria-label', customAriaLabel);
    });

    it('sets custom value for `selectMonthAriaLabel` locale', () => {
      const customAriaLabel = 'test';
      render(
        <LocaleContext.Provider value={{ locale: { PeriodPicker: { selectMonthAriaLabel: customAriaLabel } } }}>
          <PeriodPicker value={'1.2.2021'} onValueChange={jest.fn()} />
        </LocaleContext.Provider>,
      );
    ...
    it('sets custom value for `selectYearAriaLabel` locale', () => {
      const customAriaLabel = 'test';
      render(
        <LocaleContext.Provider value={{ locale: { PeriodPicker: { selectYearAriaLabel: customAriaLabel } } }}>
          <PeriodPicker value={'1.2.2021'} onValueChange={jest.fn()} />
        </LocaleContext.Provider>,
      );
    ...
    it('sets custom value for `selectChosenAriaLabel` locale', () => {
      const customAriaLabel = 'test';
      render(
        <LocaleContext.Provider value={{ locale: { PeriodPicker: { selectChosenAriaLabel: customAriaLabel } } }}>
          <PeriodPicker value={'1.2.2021'} onValueChange={jest.fn()} />
        </LocaleContext.Provider>,
      );
    ...
    it('sets custom value for `dayCellChooseDateAriaLabel` locale', () => {
      ...
      const date = '1.2.2021';
      render(
        <LocaleContext.Provider value={{ locale: { PeriodPicker: { dayCellChooseDateAriaLabel: customAriaLabel } } }}>
          <PeriodPicker value={date} onValueChange={jest.fn()} />
        </LocaleContext.Provider>,
      );
  ...
  describe('mobile', () => {
    ...
    const MobilePicker = ({ initialDate = '02.07.2017' }) => {
      ...
      return <PeriodPicker enableTodayLink width="auto" value={date} onValueChange={setDate} />;
    };

    const waitForMonth = (theMonth: number, theYear: number) => {
      const locale = PeriodPickerLocaleHelper.get();
    ...
    it('should change value from inner input', () => {
      ...
      const input = within(screen.getByTestId(PeriodPickerDataTids.input)).getByTestId(InputLikeTextDataTids.input);
    ...
    it('should change value from day select', async () => {
      ...
      expect(currentMonth).toBeDefined();
      const monthRoot = within(currentMonth as HTMLElement);
      const ariaLabel = `${PeriodPickerLocaleHelper.get().dayCellChooseDateAriaLabel}: ${new InternalDate({
        value: expectedDate,
      }).toA11YFormat()}`;
